...
cx = iris.Constraint(longitude = lambda x: 20<=x<=25)
...
plt.colorbar()
plt.ylabel("latitude")
plt.xlabel("dewpoint leads geopotential      geopotential leads dewpoint \n lag (days)")
plt.title("lead-lag correlation between upper \n level geopotential and dewpoint")
...
for i in range(8):
  plt.subplot(4,2,i+1)
  a=plt.pcolormesh(np.arange(-0.5,61),plat,zz[i].T,vmin=-1700,vmax=1700,cmap=cmap)
...
cb1.set_label("300 hPa geopotential height anomaly (m)")
